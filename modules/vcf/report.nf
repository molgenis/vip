...
process report {
  ...
  shell:
    ...
    configJsonStr = JsonOutput.toJson(new File(params.vcf.report.config).getText('UTF-8').replaceFirst("\\{", "\"vip\": {\"filter_field\": {\"type\": \"genotype\",\"name\": \"VIPC_S\"},\"params\":" + JsonOutput.toJson(params) + "},")).replaceAll ("\\)","\\\\)").replaceAll ("\\(","\\\\(")
