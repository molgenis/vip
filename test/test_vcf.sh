...
test_lp () {
  ...
  if [ "$(zcat "${OUTPUT_DIR}/vip.vcf.gz" | grep -vc "^#")" -lt 2451 ]; then
    return 1
  fi
}
...
test_lb () {
  ...
  if [ "$(zcat "${OUTPUT_DIR}/vip.vcf.gz" | grep -vc "^#")" -gt 1345 ]; then
    return 1
  fi
}
...
test_lb_b38 () {
  ...
  if [ "$(zcat "${OUTPUT_DIR}/vip.vcf.gz" | grep -vc "^#")" -gt 1269 ]; then
    return 1
  fi
}
