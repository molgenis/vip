...
APPTAINER_BIND=${BIND} apptainer exec ${VIP_DIR}/images/bcftools-1.17.sif bcftools convert -O z -o ${VIP_DIR}/test/resources/lb.vcf.gz ${VIP_DIR}/test/resources/lb.bcf.gz

APPTAINER_BIND=${BIND} apptainer exec ${VIP_DIR}/images/picard-3.1.0.sif java -jar /opt/picard/lib/picard.jar LiftoverVcf -I "${VIP_DIR}/test/resources/lb.vcf.gz" -O "${VIP_DIR}/test/resources/lb_b38.vcf.gz" -C "${VIP_DIR}/test/resources/b37ToHg38.over.chain" --REJECT "${VIP_DIR}/test/resources/lb_b38_rejected.vcf.gz" -WMC "true" -R "${VIP_DIR}/resources/GRCh38/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna.gz"

APPTAINER_BIND=${BIND} apptainer exec ${VIP_DIR}/images/bcftools-1.17.sif bcftools convert -O b -o ${VIP_DIR}/test/resources/lb_b38.bcf.gz ${VIP_DIR}/test/resources/lb_b38.vcf.gz
