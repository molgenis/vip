...
vep () {
  ...
  if [ -n "!{params.annotate_vep_plugin_spliceai_snv}" ] && [ -n "!{params.annotate_vep_plugin_spliceai_indel}" ]; then
    args+=("--plugin" "SpliceAI,snv=!{params.annotate_vep_plugin_spliceai_snv},indel=!{params.annotate_vep_plugin_spliceai_indel}")
  fi
  if [ -n "!{params.annotate_vep_plugin_vkgl}" ] && [ -n "!{params.annotate_vep_plugin_vkgl_mode}" ]; then
    args+=("--plugin" "VKGL,!{params.annotate_vep_plugin_vkgl},!{params.annotate_vep_plugin_vkgl_mode}")
  fi
  if [ -n "!{params.annotate_vep_custom_gnomad}" ]; then
    args+=("--custom" "!{params.annotate_vep_custom_gnomad},gnomAD,vcf,exact,0,AF")
  fi
