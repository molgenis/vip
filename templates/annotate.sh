...
contains_sv() {
  ...
  local vcf_header
  vcf_header=$(!{apptainer_bcftools} bcftools view -h "${vcf_path}")
...
annot_sv() {
  ...
  !{apptainer_annotsv} AnnotSV "${args[@]}"
...
capice_bcftools() {
  ...
  echo -e "${header}$(!{apptainer_bcftools} bcftools +split-vep -l "!{vcfCapiceAnnotatedPath}" | cut -f 2 | tr '\n' '\t' | sed 's/\t$//')" | cat - "${capiceInputPathHeaderless}" > "!{capiceInputPath}"
}
